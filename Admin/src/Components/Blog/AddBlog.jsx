...
const AddBlog = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (formFields.description.trim() === "") {
      context.openAlertBox("error", "Blog description is required");
      setIsLoading(false);
      return;
    }
    if (previews.length === 0) {
    ...
    postData("/api/blog/add", {
      title: formFields.title,
      images: previews, // send full previews array
      description: formFields.description,
    }).then((res) => {
      if (res?.error) {
        context.openAlertBox("error", res?.message || "Failed to add blog");
        setIsLoading(false);
      }
      if (res?.error === false) {
        setTimeout(() => {
          setIsLoading(false);
          context.openAlertBox("success", res?.message || "Blog added");
