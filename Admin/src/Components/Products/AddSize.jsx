...
const AddSize = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ ADD New RAM
    if (!editId) {
      postData(`/api/product/productSize/create`, { name: name.trim() }).then(
        (res) => {
          if (res?.error === false) {
            context.openAlertBox(
              "success",
              res?.message || "Product Size added successfully"
            );
            setName("");
            getData();
          } else {
    ...
    // ✅ UPDATE Existing RAM
      ...
      editData(`/api/product/productSize/${editId}`, {
        ...
        if (res?.error === false) {
          context.openAlertBox(
            "success",
            res?.message || "Product Size updated successfully"
          );
          setName("");
          setEditId("");
          getData();
    ...
    <>
      <div className="flex items-center justify-between px-2 py-0 mt-3">
        <h2 className="text-[18px] font-[600]">
          {editId ? "Edit Product Size" : "Add Product Size"}
        </h2>
      </div>
            ...
            <table className="w-full text-sm text-left text-gray-500">
              <thead className="text-xs text-black font-semibold uppercase bg-gray-50">
                <tr>
                  ...
                  <td className="px-0 py-2 w-[60%]">Product Size</td>
                  ...
                  >
                    ...
                    <td className="px-0 py-2">
                      <div className="flex items-center gap-4">
                        <Tooltip1 title="Edit Size" placement="top-start">
                          <Button
                        ...
                        <Tooltip1 title="Delete Size" placement="top-start">
                          <Button
                            className="!w-[35px] !h-[35px] !min-w-[35px] bg-[#f1f1f1] !border !border-[rgba(0,0,0,0.2)] !rounded-lg hover:bg-[#f1faff]"
